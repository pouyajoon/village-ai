...
const Plan: React.FC = () => {
    ...
    const [selectedLds, setSelectedLds] = useState<number[]>(lieuxDits.map(ld => ld.id));
    ...
    const filteredLieuxDits = lieuxDits.filter(ld =>
        ld.active && ld.name.toLowerCase().includes(filter.toLowerCase())
    );
    ...
    useEffect(() => {
        ...
        if (mapInstance.current) {
            ...
            const selectedMarkers = lieuxDits
                ...
                .map(ld =>
                    L.marker([ld.latitude, ld.longitude])
                        .bindPopup(`<b>${ld.name}</b><br>${ld.address}`)
                );

            if (selectedMarkers.length > 0) {
                featureGroupRef.current = L.featureGroup(selectedMarkers).addTo(mapInstance.current);
    ...
    const handleSelectAll = () => {
        setSelectedLds(lieuxDits.map(ld => ld.id));
    };
        ...
        <Box sx={{ display: 'flex', height: '100%' }}>
            ...
            >
                <LdDrawer
                    ...
                    onSelectAll={() => setSelectedLds(filteredLieuxDits.map(ld => ld.id))}
